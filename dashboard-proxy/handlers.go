...
func (r *oauthProxy) logoutHandler(w http.ResponseWriter, req *http.Request) {
	...
	if r.config.EnableLogoutRedirect {
		sendTo := fmt.Sprintf("%sv2/logout", strings.TrimSuffix(r.config.DiscoveryURL, "//.well-known/openid-configuration"))
		...
		r.redirectToURL(fmt.Sprintf("%s?client_id=%s&returnTo=%s", sendTo, url.QueryEscape(r.config.ClientID),
		    url.QueryEscape(redirectURL)), w, req, http.StatusSeeOther)
